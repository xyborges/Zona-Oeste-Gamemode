...
CMD:newcam (playerid, params[])
{
	...
	cams ++;
	if(cams >= MAX_CAMS) { SendClientMessage(playerid,c_blue,"(!) Max cameras limit excedeed!"); cams --; return 1; }
	...
	DB_FreeResultSet(DB_ExecuteQuery(db_Cam,query));
	...
	SendClientMessage(playerid,c_green,"* New camera created.");
	return 1;
}
...
{
	new camid;
	if(sscanf(params,"d",camid))return SendClientMessage(playerid,c_blue,"(!) Please type: /camint [camera id]");
	if(camid < 1 || camid > cams)return SendClientMessage(playerid,c_blue,"(!) This camera does not exist!");
...
{
	if(GetPlayerState(playerid) == 9)return SendClientMessage(playerid,c_blue,"(!) You can't edit in camera mode!");
	if(editMode[playerid] != 0)return SendClientMessage(playerid,c_blue,"(!) You are already editing something. Please finish editing or type /editstop!");
	new camid;
	if(sscanf(params,"d",camid))return SendClientMessage(playerid,c_blue,"(!) Please type: /editcam [camera id]");
	if(camid < 1 || camid > cams)return SendClientMessage(playerid,c_blue,"(!) This camera does not exist!");
	if(!IsValidDynamicObject(obj_Pos[camid][0]))return SendClientMessage(playerid,c_blue,"(!) This object is not valid!");
	SetDynamicObjectRot(obj_Pos[camid][0],0,0,0);
	new str[24];
	format(str,sizeof(str),"* Editing camera %d.",camid);
	SendClientMessage(playerid,c_green,str);
	editMode[playerid] = 1;
	editId[playerid] = camid;
...
CMD:editlook (playerid, params[])
{
	...
	if(editMode[playerid] != 0)return SendClientMessage(playerid,c_blue,"(!) You are already editing something. Please finish editing or type /editstop!");
	new camid;
	if(sscanf(params,"d",camid))return SendClientMessage(playerid,c_blue,"(!) Please type: /editlook [camera id]");
	if(camid < 1 || camid > cams)return SendClientMessage(playerid,c_blue,"(!) This camera does not exist!");
	if(!IsValidDynamicObject(obj_Look[camid][0]))return SendClientMessage(playerid,c_blue,"(!) This object is not valid!");
	new str[24];
...
CMD:ieditcam (playerid, params[])
{
	...
	if(editMode[playerid] != 0)return SendClientMessage(playerid,c_blue,"(!) You are already editing something. Please finish editing or type /editstop!");
	new camid;
	if(sscanf(params,"d",camid))return SendClientMessage(playerid,c_blue,"(!) Please type: /ieditcam [camera id]");
	if(camInfo[camid][c_Interpolated] == 1 || camInfo[camid][c_Interpolated] == 3)
	{
		if(camid < 1 || camid > cams)return SendClientMessage(playerid,c_blue,"(!) This camera does not exist!");
		if(!IsValidDynamicObject(obj_Pos[camid][1]))return SendClientMessage(playerid,c_blue,"(!) This object is not valid!");
...
CMD:ieditlook (playerid, params[])
{
	...
	if(editMode[playerid] != 0)return SendClientMessage(playerid,c_blue,"(!) You are already editing something. Please finish editing or type /editstop!");
	new camid;
	if(sscanf(params,"d",camid))return SendClientMessage(playerid,c_blue,"(!) Please type: /ieditlook [camera id]");
	if(camInfo[camid][c_Interpolated] == 2 || camInfo[camid][c_Interpolated] == 3)
	{
		if(camid < 1 || camid > cams)return SendClientMessage(playerid,c_blue,"(!) This camera does not exist!");
		if(!IsValidDynamicObject(obj_Look[camid][1]))return SendClientMessage(playerid,c_blue,"(!) This object is not valid!");
...
{
	new camid, ammount;
	if(sscanf(params,"dd",camid,ammount))return SendClientMessage(playerid,c_blue,"(!) Please type: /camtime [camera id][time(seconds)]");
	if(camInfo[camid][c_Interpolated] == 1 || camInfo[camid][c_Interpolated] == 3)
	{
	    if(camid < 1 || camid > cams)return SendClientMessage(playerid,c_blue,"(!) This camera does not exist!");
	    camInfo[camid][c_Postime] = ammount*1000;
	    SendClientMessage(playerid,c_green,"* Camera position interpolation time updated.");
	    SaveCamera(camid,0);
	}
	else SendClientMessage(playerid,c_blue,"(!) You camera needs to be in interpolation mode 2 or 4!");
	return 1;
}
...
{
	new camid, ammount;
	if(sscanf(params,"dd",camid,ammount))return SendClientMessage(playerid,c_blue,"(!) Please type: /looktime [camera id][time(seconds)]");
	if(camInfo[camid][c_Interpolated] == 2 || camInfo[camid][c_Interpolated] == 3)
	{
	    if(camid < 1 || camid > cams)return SendClientMessage(playerid,c_blue,"(!) This camera does not exist!");
	    camInfo[camid][c_Looktime] = ammount*1000;
	    SendClientMessage(playerid,c_green,"* Camera look at interpolation time updated.");
	    SaveCamera(camid,0);
	}
	else SendClientMessage(playerid,c_blue,"(!) You camera needs to be in interpolation mode 3 or 4!");
	return 1;
}
...
{
	new camid;
	if(sscanf(params,"d",camid))return SendClientMessage(playerid,c_blue,"(!) Please type: /playcam [camera id]");
	if(camid < 1 || camid > cams)return SendClientMessage(playerid,c_blue,"(!) This camera does not exist!");
	if(GetPlayerState(playerid) == 0)return SendClientMessage(playerid,c_blue,"(!) Invalid state!");
...
{
	new camid;
	if(sscanf(params,"d",camid))return SendClientMessage(playerid,c_blue,"(!) Please type: /exportcam [camera id]");
	if(camid < 1 || camid > cams)return SendClientMessage(playerid,c_blue,"(!) This camera does not exist!");
	new str[256];
	...
	Export("=================================================================");
	SendClientMessage(playerid,c_green,"* Camera code lines exported. Please check scriptfiles/cameraexports.txt!");
	return 1;
}
...
{
	#pragma unused params
	SendClientMessage(playerid,c_green,"* Camera editor has been stoped.");
	editMode[playerid] = 0;
	return 1;
}
...
public LoadCamDb()
{
	...
	DB_FreeResultSet(DB_ExecuteQuery(db_Cam,"CREATE TABLE IF NOT EXISTS `cameras` (`camid`,`interpolated`,`posx`,`posy`,`posz`,`intposx`,`intposy`,`intposz`,`postime`,`lookx`,`looky`,`lookz`,`intlookx`,`intlooky`,`intlookz`,`looktime`)"));
	...
	for(new i=1; i < MAX_CAMS; i++)
	{
        ...
        res = DB_ExecuteQuery(db_Cam,query);
        ...
        if(DB_GetRowCount(res))
        {
            ...
            DB_GetFieldStringByName(res, "interpolated", field, 50 );
            camInfo[i][c_Interpolated] = strval(field);
            DB_GetFieldStringByName(res, "postime", field, 50 );
            camInfo[i][c_Postime] = strval(field);
            DB_GetFieldStringByName(res, "looktime", field, 50 );
            camInfo[i][c_Looktime] = strval(field);
            
            DB_GetFieldStringByName(res, "posx", field, 50 );
            camInfo[i][c_Posx] = floatstr(field);
            DB_GetFieldStringByName(res, "posy", field, 50 );
            camInfo[i][c_Posy] = floatstr(field);
            DB_GetFieldStringByName(res, "posz", field, 50 );
            camInfo[i][c_Posz] = floatstr(field);
            DB_GetFieldStringByName(res, "intposx", field, 50 );
            camInfo[i][c_Intposx] = floatstr(field);
            DB_GetFieldStringByName(res, "intposy", field, 50 );
            camInfo[i][c_Intposy] = floatstr(field);
            DB_GetFieldStringByName(res, "intposz", field, 50 );
            camInfo[i][c_Intposz] = floatstr(field);

            DB_GetFieldStringByName(res, "lookx", field, 50 );
            camInfo[i][c_Lookx] = floatstr(field);
            DB_GetFieldStringByName(res, "looky", field, 50 );
            camInfo[i][c_Looky] = floatstr(field);
            DB_GetFieldStringByName(res, "lookz", field, 50 );
            camInfo[i][c_Lookz] = floatstr(field);
            DB_GetFieldStringByName(res, "intlookx", field, 50 );
            camInfo[i][c_Intlookx] = floatstr(field);
            DB_GetFieldStringByName(res, "intlooky", field, 50 );
            camInfo[i][c_Intlooky] = floatstr(field);
            DB_GetFieldStringByName(res, "intlookz", field, 50 );
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == d_Cam)
	{
	    if(!response) return editId[playerid] = -1;
	        ...
	        SendClientMessage(playerid,c_green,"* Interpolation updated.");
	...
	if(dialogid == d_Cam+1)
	{
	    if(response)
		{
	    	if(listitem == 0) SendClientMessage(playerid,c_yellow,"» /newcam - This command is used to create a new camera.");
	    	else if(listitem == 1) SendClientMessage(playerid,c_yellow,"» /camint - This will let you change the interpolation state of your camera.");
			else if(listitem == 2) SendClientMessage(playerid,c_yellow,"» /editcam - This will display the edit UI, wich you can use to change camera's position.");
			else if(listitem == 3) SendClientMessage(playerid,c_yellow,"» /editlook - This will display the edit UI, wich you can use to change the position where your camera is looking.");
			else if(listitem == 4) SendClientMessage(playerid,c_yellow,"» /ieditcam - This will display the edit UI, wich you can use to change interpolated camera's position.");
			else if(listitem == 5) SendClientMessage(playerid,c_yellow,"» /ieditlook - This will display the edit UI, wich you can use to change the interpolated position where your camera is gonna look.");
			else if(listitem == 6) SendClientMessage(playerid,c_yellow,"» /camtime - This edits the camera position interpolation time.");
			else if(listitem == 7) SendClientMessage(playerid,c_yellow,"» /looktime - This edits the camera look at interpolation time.");
			else if(listitem == 8) SendClientMessage(playerid,c_yellow,"» /playcam - This will play the camera animation.");
			else if(listitem == 9) SendClientMessage(playerid,c_yellow,"» /stopcam - This will get you out of camera mode.");
			else if(listitem == 10) SendClientMessage(playerid,c_yellow,"» /exportcam - This will export the code lines you need to implement a camera animation in your script.");
			else if(listitem == 11) SendClientMessage(playerid,c_yellow,"» /editstop - In case the camera edit is stuck on, this will help fix the problem.");
	    }
	}
	return 1;
}
...
public OnPlayerEditDynamicObject(playerid, objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz)
{
	if(editMode[playerid] != 0)
	{
	    ...
	    if(response == 0) { SetDynamicObjectPos(objectid,oldPos[0],oldPos[1],oldPos[2]); UpdateGizmo(0,editId[playerid]); UpdateGizmo(1,editId[playerid]); editMode[playerid] = 0; editId[playerid] = -1; SendClientMessage(playerid,c_green,"* Edit mode canceld."); }
	    if(response == 1)
	    {
	        if(editMode[playerid] == 1)
	        {
	            ...
	            SendClientMessage(playerid,c_green,"* Camera position updated.");
	            UpdateGizmo(0,camid);
	            SaveCamera(camid,1);
	        ...
	        else if(editMode[playerid] == 2)
	        {
	            ...
	            SendClientMessage(playerid,c_green,"* Camera look at updated.");
	            UpdateGizmo(0,camid);
	            SaveCamera(camid,2);
	            editMode[playerid] = 0; editId[playerid] = -1;
	        }
	        else if(editMode[playerid] == 3)
	        {
	            ...
	            SendClientMessage(playerid,c_green,"* Interpolated camera position updated.");
	            UpdateGizmo(1,camid);
	            SaveCamera(camid,1);
	            editMode[playerid] = 0; editId[playerid] = -1;
	        }
	        else if(editMode[playerid] == 4)
	        {
	            ...
	            SendClientMessage(playerid,c_green,"* Camera interpolated look at updated.");
	            UpdateGizmo(1,camid);
	            SaveCamera(camid,2);
	            editMode[playerid] = 0; editId[playerid] = -1;
	        }
